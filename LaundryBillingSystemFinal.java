...
System.out.println("╚═╝     ╚═╝  ╚═╝╚══════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝ ╚═╝  ╚═╝  ╚═══╝  ╚══════╝");
System.out.println("===========================================================================");
System.out.println("                         Laundry Billing System                           ");
System.out.println("===========================================================================\n");
...
//the condition (y/n)
		...
		while (true) {
			...
			if (promo.isEmpty()) {
				...
				discount = 50;
				appliedPromo = promo;
				System.out.println("Discount Applied! 50 Php Discount.");
				break;
...
while (!confirmed) {
    ...
    System.out.println("[8] Discount Code   : " + (discount > 0 ? promo : "None"));
    System.out.println("=====================================");
    ...
    if (yesNo("\nIs all information correct? ")) {
        ...
        switch (editChoice) {
            ...
            case 8:
                ...
                while (true) {
                    System.out.println("\nDISCOUNT CODES: 10NTHFRESHWAVEANNIVERSARY / FSstudent / NEWCUSTOMER");
                    System.out.print("Enter Discount Code (or press Enter to remove discount code): ");

                    String newPromo = sc.nextLine().trim();
                    double tempTotal = baseTotal + ironingFee + expressFee;
                    
                    if (newPromo.isEmpty()) {
    if (yesNo("Are you sure you want to remove discount code? ")) {
        promo = "";
        discount = 0;
    ...
    break;
					...
					if (newPromo.equals(appliedPromo)){
						System.out.println("Discount Code is already used.");
						break;
					}
      ...
      if (promo.equalsIgnoreCase("10NTHFRESHWAVEANNIVERSARY")) {
    discount = tempTotal * 0.10;
} else if (promo.equalsIgnoreCase("NEWCUSTOMER")) {
    discount = tempTotal * 0.05;
} else if (promo.equalsIgnoreCase("FSstudent")) {
    discount = 50;
} else {
    discount = 0;
}
		...
		System.out.println("Total               : " + (baseTotal + ironingFee + expressFee) + " Php");
		...
		while (true) {
			...
			if (input.isEmpty()) {
				System.out.println("Payment cannot be empty. Please enter a valid amount.\n");
				continue;
			}
			...
			try {
				...
				if (payment <= 0) {
					System.out.println("Invalid input. Payment must be greater than 0. Please try again.\n");
...
//to track the specific id in transactions arraylist, if the id matched the user's input, then it can be update'
    for (Transaction t : transactions) {
        if (t.id == id) {
            ...
            if (t.status.equalsIgnoreCase("Pending")) {
                if (yesNo("Would you like to update this transaction ID " + t.id  + " to 'Ready'? ")) {
                    t.status = "Ready";
                ...
                if (yesNo("Would you like to update this transaction ID " + t.id  + " to 'Claimed'? ")) {
                    t.status = "Claimed";
	...
	static void deleteTransaction() {
		...
		while (iterator.hasNext()) {
			...
			if (t.id == id) {
				if (yesNo("Are you sure you want to delete ID " + t.id + "? ")) {
					iterator.remove();
